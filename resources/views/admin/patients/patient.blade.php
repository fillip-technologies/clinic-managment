...
<style>
    ...
    <div class="w-full max-w-7xl bg-white rounded-3xl card-shadow p-6 md:p-9 transition-all m-auto">
        ...
        <form action="{{ route('store.patient') }}" id="patientForm" class="space-y-7" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5">
                    ...
                    <div class="col-span-1">
                        @php
                            $initialDob = old('dob', request('dob'));
                            if (empty($initialDob) && request()->filled('age') && is_numeric(request('age')) && request('age') > 0) {
                                $initialDob = (date('Y') - intval(request('age'))) . '-01-01';
                            }
                        @endphp
                        ...
                        <input type="date" name="dob" id="patientDobInput" max="{{ date('Y-m-d') }}"
                            value="{{ $initialDob }}" onchange="calculateDobAge(this.value)"
            ...
            @php
                $hasDiabetes = (
                    ...
                    !empty(old('duration_of_diabetes')) ||
                ...
                <div id="diabetes_fields" class="{{ $hasDiabetes ? '' : 'hidden' }}">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        ...
                        <div><label class="block text-xs font-semibold uppercase text-[#2f5a77] mb-1"><i
                            ...
                            <input type="text" name="diabetes_duration" value="{{ old('diabetes_duration', old('duration_of_diabetes')) }}" placeholder="e.g. 5 yrs"
                                class="w-full rounded-xl border border-[#d3dfea] px-4 py-2.5 text-sm bg-[#fafdff] input-focus">
                        </div>
                        <div><label class="block text-xs font-semibold uppercase text-[#2f5a77] mb-1"><i
            ...
            <div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                    ...
                    <div><label class="block text-xs font-semibold text-[#2f5a77]">Temperature (°F)</label>
                        <input type="number" step="0.01" min="0" max="150" name="temprature" value="{{ old('temprature') }}" placeholder="e.g. 98.60"
                            class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus"></div>
                </div>
            </div>
