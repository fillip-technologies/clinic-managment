...
@endphp
    ...
    <div class="w-full max-w-7xl bg-white rounded-3xl card-shadow p-6 md:p-9 transition-all m-auto">
        ...
        <form action="{{ route('patient.update', $patient->id ?? $record->patient_id) }}" id="editPatientForm" method="POST" enctype="multipart/form-data">
            @csrf
                        ...
                        <select onchange="window.location.href = '{{ route('patient.edit', $patient->id) }}?record_id=' + this.value"
                            ...
                            @foreach ($allRecords as $idx => $rec)
                                <option value="{{ $rec->id }}" {{ $rec->id == $record->id ? 'selected' : '' }}>
                                    Visit #{{ $allRecords->count() - $idx }}: {{ ($rec->record_date ? \Carbon\Carbon::parse($rec->record_date)->format('d M Y') : ($rec->created_at ? $rec->created_at->format('d M Y') : 'Visit #' . $rec->id)) }} {{ $loop->first ? '— (Latest Visit)' : '' }}
                                </option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div>
                    ...
                    <div class="col-span-1">
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="text" name="patient_name"
                                value="{{ old('patient_name', $patient->patient_name ?? ($record->patient->patient_name ?? '')) }}"
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="number" name="age"
                                value="{{ old('age', $patient->age ?? ($record->patient->age ?? '')) }}"
                        ...
                        <div class="col-span-1">
                            ...
                            <select name="gender"
                                ...
                                <option value="Male"
                                    {{ old('gender', $patient->gender ?? ($record->patient->gender ?? '')) == 'Male' ? 'selected' : '' }}>Male
                                </option>
                                <option value="Female"
                                    {{ old('gender', $patient->gender ?? ($record->patient->gender ?? '')) == 'Female' ? 'selected' : '' }}>Female
                                </option>
                                <option value="Other"
                                    {{ old('gender', $patient->gender ?? ($record->patient->gender ?? '')) == 'Other' ? 'selected' : '' }}>Other
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="text" name="guardian_name"
                                value="{{ old('guardian_name', $patient->father_husband_name ?? ($record->patient->father_husband_name ?? '')) }}"
                                placeholder="Name"
                                class="w-full rounded-xl border border-[#d3dfea] px-4 py-2.5 text-sm bg-white input-focus">
                        </div>
                        ...
                        <div class="col-span-1">
                            ...
                            <select name="rcdho_grade"
                                ...
                                <option value="Grade I"
                                    {{ old('rcdho_grade', $patient->rcdho_grade ?? ($record->patient->rcdho_grade ?? '')) == 'Grade I' ? 'selected' : '' }}>
                                    Grade I</option>
                                <option value="Grade II"
                                    {{ old('rcdho_grade', $patient->rcdho_grade ?? ($record->patient->rcdho_grade ?? '')) == 'Grade II' ? 'selected' : '' }}>
                                    Grade II</option>
                                <option value="Grade III"
                                    {{ old('rcdho_grade', $patient->rcdho_grade ?? ($record->patient->rcdho_grade ?? '')) == 'Grade III' ? 'selected' : '' }}>
                                    Grade III</option>
                            </select>
                        </div>
                        ...
                        <div class="col-span-1 sm:col-span-2">
                            ...
                            <input type="text" name="address"
                                value="{{ old('address', $patient->address ?? ($record->patient->address ?? '')) }}"
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="tel" name="mobile"
                                value="{{ old('mobile', $patient->mobile_no ?? ($record->patient->mobile_no ?? '')) }}"
                ...
                @php
                    $hasDiabetes = (
                        ...
                        !empty($record->c_peptide) ||
                        !empty($record->insulin_antibodies) ||
                        !empty($record->mody_biomarkers) ||
                ...
                <div>
                    ...
                    <div id="diabetes_fields" class="{{ $hasDiabetes ? '' : 'hidden' }}">
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                <label class="block text-xs font-semibold uppercase text-[#2f5a77] mb-1">
                                    <i class="fas fa-flask"></i> C PEPTIDE
                                </label>
                                <input type="text" name="c_peptide"
                                    value="{{ old('c_peptide', $record->c_peptide ?? '') }}"
                            ...
                            <div>
                                <label class="block text-xs font-semibold uppercase text-[#2f5a77] mb-1">
                                    <i class="fas fa-shield-virus"></i> Insulin antibodies
                                </label>
                                <input type="text" name="insulin_antibodies"
                                    value="{{ old('insulin_antibodies', $record->insulin_antibodies ?? '') }}"
                            ...
                            <div>
                                <label class="block text-xs font-semibold uppercase text-[#2f5a77] mb-1">
                                    <i class="fas fa-dna"></i> Mody Biomarkers
                                </label>
                                <input type="text" name="mody_biomarkers"
                                    value="{{ old('mody_biomarkers', $record->mody_biomarkers ?? '') }}"
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                        <div>
                            ...
                            <input type="number" step="0.1" name="height"
                                value="{{ old('height', $record->height_cm ?? '') }}" placeholder="cm"
                        ...
                        <div>
                            ...
                            <select name="bmi_group"
                                ...
                                <option value="Overweight"
                                    {{ old('bmi_group', $record->bmi_group ?? '') == 'Overweight' ? 'selected' : '' }}>
                                    Overweight</option>
                                <option value="Obese"
                                    {{ old('bmi_group', $record->bmi_group ?? '') == 'Obese' ? 'selected' : '' }}>Obese
                                </option>
                            </select>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="waist_hip_ratio"
                                value="{{ old('waist_hip_ratio', $record->waist_hip_ratio ?? '') }}" placeholder="0.0"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                    </div>
                </div>
                        ...
                        <div>
                            ...
                            <select name="social_class"
                                ...
                                <option value="Upper"
                                    ...
                                    Upper</option>
                                <option value="Middle"
                                    {{ old('social_class', $record->social_class ?? '') == 'Middle' ? 'selected' : '' }}>
                                    Middle</option>
                                <option value="Lower"
                                    {{ old('social_class', $record->social_class ?? '') == 'Lower' ? 'selected' : '' }}>
                                    Lower</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="income_class"
                                ...
                                <option value="Medium"
                                    {{ old('income_class', $record->income_class ?? '') == 'Medium' ? 'selected' : '' }}>
                                    Medium</option>
                                <option value="Low"
                        ...
                        <div>
                            ...
                            <select name="physical_activity"
                                ...
                                <option value="Moderate"
                                    {{ old('physical_activity', $record->physical_activity ?? '') == 'Moderate' ? 'selected' : '' }}>
                                    Moderate</option>
                                <option value="Active"
                                    {{ old('physical_activity', $record->physical_activity ?? '') == 'Active' ? 'selected' : '' }}>
                                    Active</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="veg_nonveg"
                                ...
                                <option value="Non-vegetarian"
                                    {{ old('veg_nonveg', $record->veg_nonveg ?? '') == 'Non-vegetarian' ? 'selected' : '' }}>
                                    Non-vegetarian</option>
                                <option value="Vegan"
                                    {{ old('veg_nonveg', $record->veg_nonveg ?? '') == 'Vegan' ? 'selected' : '' }}>Vegan
                                </option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                        ...
                        <div>
                            ...
                            <input type="text" name="creatinine"
                                value="{{ old('creatinine', $record->creatinine ?? '') }}" placeholder="mg/dL"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-[#2f5a77]">EGFr</label>
                            <input type="text" name="egfr" value="{{ old('egfr', $record->egfr ?? '') }}"
                                placeholder="mL/min"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-[#2f5a77]">ACR</label>
                            <input type="text" name="acr" value="{{ old('acr', $record->acr ?? '') }}"
                                placeholder="mg/g"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                            ...
                            <input type="text" name="uric_acid"
                                value="{{ old('uric_acid', $record->uric_acid ?? '') }}" placeholder="mg/dL"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-[#2f5a77]">Urine cast cell</label>
                            <input type="text" name="urine_cast_cell"
                                value="{{ old('urine_cast_cell', $record->urine_cast_cell ?? '') }}" placeholder="cells"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-[#2f5a77]">Na+</label>
                            <input type="text" name="sodium" value="{{ old('sodium', $record->na_plus ?? '') }}"
                                placeholder="mEq/L"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="fib_score"
                                value="{{ old('fib_score', $record->fib_score ?? '') }}" placeholder="score"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-[#2f5a77]">Fib Scan</label>
                            <input type="text" name="fib_scan" value="{{ old('fib_scan', $record->fib_scan ?? '') }}"
                                placeholder="kPa"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="hba1c" value="{{ old('hba1c', $record->hba1c ?? '') }}"
                                placeholder="%"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-[#2f5a77]">TSH</label>
                            <input type="text" name="tsh" value="{{ old('tsh', $record->tsh ?? '') }}"
                                placeholder="µIU/mL"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-[#2f5a77]">T3</label>
                            <input type="text" name="t3" value="{{ old('t3', $record->t3 ?? '') }}"
                                placeholder="ng/dL"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-[#2f5a77]">T4</label>
                            <input type="text" name="t4" value="{{ old('t4', $record->t4 ?? '') }}"
                                placeholder="µg/dL"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                            ...
                            <input type="text" name="vitamin_d25"
                                value="{{ old('vitamin_d25', $record->vitamin_d25 ?? '') }}" placeholder="ng/mL"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-[#2f5a77]">Vitamin B12</label>
                            <input type="text" name="vitamin_b12"
                                value="{{ old('vitamin_b12', $record->vitamin_b12 ?? '') }}" placeholder="pg/mL"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-[#2f5a77]">S.CORTISOL</label>
                            <input type="text" name="cortisol" value="{{ old('cortisol', $record->s_cortisol ?? '') }}"
                                placeholder="µg/dL"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>
                        <div>
                            ...
                            <input type="text" name="dex_suppression_test"
                                value="{{ old('dex_suppression_test', $record->dex_skip_test ?? '') }}"
                                placeholder="result"
                                class="w-full rounded-xl border border-[#d3dfea] px-3 py-2 text-sm input-focus">
                        </div>

                        <div><label class="block text-xs font-semibold text-[#2f5a77]">Temperature (°F)</label>
                            <input type="number" step="0.01" min="0" max="150" name="temprature" value="{{ old('temprature' ,$record->temprature) }}"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="text" name="ophthalmic_exam"
                                value="{{ old('ophthalmic_exam', $record->ophthalmic_ex ?? '') }}"
                                placeholder="Findings"
                        ...
                        <div>
                            ...
                            <input type="text" name="foot_exam"
                                value="{{ old('foot_exam', $record->foot_ev ?? '') }}" placeholder="Foot exam"
                                class="w-full rounded-xl border border-[#d3dfea] px-4 py-2.5 text-sm input-focus">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-[#2f5a77]">
                            ...
                            <input type="text" name="echo_exam"
                                value="{{ old('echo_exam', $record->car_echo_ev ?? '') }}" placeholder="Echo summary"
                                class="w-full rounded-xl border border-[#d3dfea] px-4 py-2.5 text-sm input-focus">
                        </div>
                    </div>
                </div>
